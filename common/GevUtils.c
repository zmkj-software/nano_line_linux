...
int IsGevPixelTypeX11Displayable(int format)
{
	...
	switch(format)
	{
      ...
      case fmt_PFNC_BiColorBGRG10p:
      case fmt_PFNC_BiColorBGRG12p:
      case fmt_PFNC_BiColorRGBG10p:
      case fmt_PFNC_BiColorRGBG12p:
			displayable = FALSE;
			break;
		default:
			displayable = TRUE;
...
//  or BiColor encoded format. This format can then be used by a Bayer/BiColor conversion function).
//
int GetX11DisplayablePixelFormat( int convertBayer, UINT32 rawGevPixelFormat, u_int32_t *convertedGevPixelFormat, u_int32_t *displayableSaperaPixelFormat)
{
	...
	{
		// Our X11 functions will not display this converted pixel type directly 
		// (YUV or other format)- need to set up to convert it.
		if (GevIsPixelTypeRGB(rawGevPixelFormat))
		{
			// Is it a packed color image (various YUV ones) - Convert to RGB8888.
			// Set up the output formats.			
			if ( GevIsPixelTypeYUV(rawGevPixelFormat) )
			{
				// Its a YUV or other color encoded format (non-RGB, non-Mono)
				// Indicate it needs to be converted to color.
				cvtPixFmt = fmtBGRA8Packed; // Default (RGB888 in 32 bits).				
			}
			else if ( GevIsPixelTypeBiColor(rawGevPixelFormat) )
			{
				// Is it a BiColor format -Convert to RGB8888
				cvtPixFmt = fmtBGRA8Packed; // Default (RGB888 in 32 bits).				
			}
		...
		{
			// Not color and not displayable - defaults to Mono8
			if (convertedGevPixelFormat != NULL)
			{
				*convertedGevPixelFormat = cvtPixFmt;
			}
	...
	else
	{
		...
		//
		// Check if it needs decoding (i.e. Bayer/BiColor with convertBayer enabled)
		//
		if ( GevIsPixelTypeBayer(cvtPixFmt) || GevIsPixelTypeBiColor(cvtPixFmt))
		{
			if (convertBayer)
			{
				// Convert Bayer/Bicolor to BGRA8 for display (same as CORDATA_FORMAT_RGB8888)
				cvtPixFmt = fmtBGRA8Packed;
			}
		}
		else if ( GevIsPixelTypeRGB(cvtPixFmt) && (GevGetPixelDepthInBits(cvtPixFmt) > 8) )
		{ 
			// Converted output is 10 or 12 bit RGB - need to convert to 8 bit.
      ...
      switch (GevDataFormat)
      {
         ...
         case fmtRGB10V2Packed:
			...
			case fmtYUV411packed:
				...
				break;
			case fmtYUV422packed:
				format = CORDATA_FORMAT_UYVY;
				break;
			case fmt_PFNC_YUV422_8:  
				format = CORDATA_FORMAT_YUYV;
				break;
			case fmtYUV444packed:
				format = CORDATA_FORMAT_IYU2;
				break;
         default:
...
static void Convert_YUV422_YUYV_To_Mono(int pixelCount, void *in, int outDepth, void *out)
{
	int i;
	if ((in != NULL) && (out != NULL))
	{
...
static void Convert_YUV422_YUYV_To_RGB8x(int alpha_channel, int pixelCount, void *in, int outDepth, void *out)
{
	unsigned char *pIn = (unsigned char *)in;
	unsigned char *pOut = (unsigned char *)out;
	int i;
...
static void Convert_YUV422_YUYV_To_RGB888(int pixelCount, void *in, int inDepth, void *out)
{
	Convert_YUV422_YUYV_To_RGB8x(FALSE, pixelCount, in, inDepth, out);
}
static void Convert_YUV422_YUYV_To_RGB8888(int pixelCount, void *in, int inDepth, void *out)
{
	Convert_YUV422_YUYV_To_RGB8x(TRUE, pixelCount, in, inDepth, out);
}
...
void Convert_BiColor88toRGB8888( int w, int h, void *in, void *out, UINT32 alignmentBlueGreen)
{
	...
	if ( (pIn != NULL) && (pOut != NULL))
	{
		...
		for (i = 0; i < h; i++)
		{
			CorUtilConvertBicolor88toRGB8888( pIn, pOut, (SIZE_T)w, alignmentBlueGreen);
			pOut += outPitch;
			pIn  += inPitch;
		}
	}
...
void Convert_BiColor1616toRGB8888( int w, int h, void *in, void *out, UINT32 range, UINT32 alignmentBlueGreen)
{
	...
	if ( (pIn != NULL) && (pOut != NULL))
	{
		...
		for (i = 0; i < h; i++)
		{
			CorUtilConvertBicolor1616toRGB8888( pIn, pOut, (SIZE_T)w, range, alignmentBlueGreen);
			pOut += outPitch;
			pIn  += inPitch;
		}
	}
			...
			case CORX11_DATA_FORMAT_MONO:
				{
					switch(gev_format)
					{
						...
						case fmt_PFNC_YUV422_8:
							numPixels = w*h;
							Convert_YUV422_YUYV_To_RGB888(numPixels, gev_input_data, x11_depth, x11_output_data);
							break;
						case fmtYUV444packed:
							numPixels = w*h;
			...
			case CORX11_DATA_FORMAT_RGB8888:
				{
					...
					switch(gev_format)
					{
						...
						case fmtYUV422packed:
							...
							break;
						case fmt_PFNC_YUV422_8:
							Convert_YUV422_YUYV_To_RGB8888(numPixels, gev_input_data, x11_depth, x11_output_data);
							break;
						case fmtYUV444packed:
	...
	if ((gev_input_data != NULL) && (rgb_output_data != NULL))
	{
		...
		switch(gev_format)
		{
			...
			case fmtYUV422packed:
				...
				break;
			case fmt_PFNC_YUV422_8:
				Convert_YUV422_YUYV_To_RGB8888(numPixels, gev_input_data, outdepth, rgb_output_data);
				break;
			case fmtYUV444packed:
	...
	if ((gev_input_data != NULL) && (rgb_output_data != NULL))
	{
		...
		switch(gev_format)
		{
			...
			case fmtYUV422packed:
				...
				break;
			case fmt_PFNC_YUV422_8:
				Convert_YUV422_YUYV_To_RGB888(numPixels, gev_input_data, outdepth, rgb_output_data);
				break;
			case fmtYUV444packed:
